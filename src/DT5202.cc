...
int DT5202::Init(const char* addr)
{
  int res;
  uint32_t data = 0;

  LLeth_OpenDevice(addr);
  
  Fill_BoardInfo(addr);
  std::cout << std::endl << "Board PID:" << BoardInfo.pid << " initialized at " << addr << std::endl;
  printf("\tBoard Model: %s\n", BoardInfo.model);
  printf("\tBoard Channels: %d\n", BoardInfo.ch_count);
  printf("\tFirmware revision: %d\n\n", BoardInfo.fw_rev);

  LLeth_WriteRegister(a_commands, CMD_CFG_ASIC);

  Run_Start(CMD_ACQ_START, 4);

  sleep(5);

  Run_Stop(CMD_ACQ_STOP, 0);
   
  return 0;
}

void DT5202::Fill_BoardInfo(const char* addr)
{
  int res;
  uint32_t data = 0;

  res = LLeth_ReadRegister(a_pid, &data);
  BoardInfo.pid = (uint32_t)data;
  strcpy(BoardInfo.model, "DT5202");
  BoardInfo.ch_count = 64;
  res = LLeth_ReadRegister(a_fw_rev, &data);
  BoardInfo.fw_rev = (uint32_t)data;
}

void DT5202::Get_BoardState(const char* addr)
{ 
  int res;
  uint32_t data = 0;

  res = LLeth_ReadRegister(a_acq_status, &data);
  BoardInfo.pid = (uint32_t)data;
  BoardInfo.ch_count = 64;
  res = LLeth_ReadRegister(a_fw_rev, &data);
  BoardInfo.fw_rev = (uint32_t)data;
...
void DT5202::Run_Start(uint32_t comm, uint32_t set_acq)
{
  int res, snd;
  uint32_t data = 0;
  
  LLeth_WriteRegister(a_acq_ctrl, set_acq);

  res = LLeth_ReadRegister(a_acq_ctrl, &data);
  std::cout << "Acquisition status set to: " << (uint32_t)data << " - run started." << std::endl;

  LLeth_WriteRegister(a_commands, comm);
}

void DT5202::Run_Stop(uint32_t comm, uint32_t set_acq)
{
  int res, snd;
  uint32_t data = 0;
  ...
  res = LLeth_ReadRegister(a_acq_ctrl, &data);
  std::cout << "Acquisition status reset to: " << (uint32_t)data << " - run stopped."<< std::endl;
